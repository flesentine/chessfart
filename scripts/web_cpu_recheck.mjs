...
const outDir = 'build/web-cpu-recheck';
...
async function latestHistory(page) {
  ...
  const lines = [];
  for (let i = Math.max(0, count - 3); i < count; i++) lines.push(await historyLine(page, i));
  return lines;
}
...
async function playGame(browser, label, difficulty, seed) {
  ...
  await page.goto(`http://127.0.0.1:8127/?review=${label}`,{waitUntil:'domcontentloaded',timeout:15000});
  ...
  while (terminal < 2 && turns < 120) {
    ...
    const lines = await latestHistory(page);
    for (const line of lines) {
      if (line.startsWith('CPU FART')) {
        cpuFarts++;
        if (line.includes('PUSH')) cpuPushes++;
        fartLines.push(`${turns+1}: ${line}`);
      }
      if (line.startsWith('YOU FART')) humanFarts++;
    }
    turns++;
    terminal = await call(page,'cf_review_status');
...
try {
  ...
  fs.writeFileSync(path.join(outDir,'recheck.txt'),summary.join('\n')+'\n');
  console.log(summary.join('\n'));
  if (totalCpuFarts < 2) throw new Error(`CPU fart frequency too low across recheck: ${totalCpuFarts}`);
  if (totalPushes < 1) throw new Error('CPU produced no PUSH across recheck');
} finally {
