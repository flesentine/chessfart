                      ...
                      unsigned sample_rate)
{
    ...
    fp = fopen("build/host/chessfart_build11_fart.wav", "wb");
    if (fp == 0) return;
    fwrite("RIFF", 1, 4, fp);
...
{
    FILE *fp = fopen("build/host/chessfart_build11_audio.log", "a");
    if (fp == 0) return;
    fputs(text, fp);
    fputc('\n', fp);
    fclose(fp);
...
CfAudioDevice audio_platform_init(CfAudioDevice requested)
{
    ...
    fp = fopen("build/host/chessfart_build11_audio.log", "w");
    if (fp != 0) {
