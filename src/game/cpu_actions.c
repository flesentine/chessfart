...
static void generate_actions_core(const CfBoard *board,
        ...
        for (file = 0; file < 8; ++file) {
            ...
            if (have_actor_king)
                board_generate_legal_moves_prelocated_scratch(
                    board, &action_scratch, file, rank,
                    actor_king_file, actor_king_rank, &moves);
            else
                moves.count = 0;
            ...
            gas_scan_farts_prechecked_scratch(
                board, gas, &action_scratch, file, rank,
...
void cpu_generate_actions(const CfBoard *board, const CfGasState *gas,
                          CfCpuActionList *list)
{
    generate_actions_core(board, gas, list, 0);
}
