...
static volatile int eval_breakdown_profile_sink;

static int profile_eval_piece_value(CfPieceType t)
{
...
static int profile_eval_in_bounds(int file, int rank)
{
    return file >= 0 && file < 8 && rank >= 0 && rank < 8;
}

static int profile_eval_pressure(const CfBoard *board,
                                 int file, int rank,
                                 CfPieceColor color)
{
                                        ...
                                        CfPieceColor color)
{
    ...
    for (i = 0; i < 8; ++i) {
        ...
        push_file = target_file + df[i];
        push_rank = target_rank + dr[i];
        if (!profile_eval_in_bounds(target_file, target_rank) ||
            !profile_eval_in_bounds(push_file, push_rank))
            continue;
        target = &board->squares[target_rank][target_file];
        if (target->type == CF_PIECE_NONE ||
            board->squares[push_rank][push_file].type != CF_PIECE_NONE)
        ...
        score += target->color != color ?
            8 + profile_eval_piece_value(target->type) / 80 : 2;
    }
    return score;
}

static int profile_eval_base_scan(const CfBoard *board,
                                  const CfGasState *gas)
{
    const CfPiece *piece;
    int file;
    ...
    for (rank = 0; rank < 8; ++rank) {
        for (file = 0; file < 8; ++file) {
            ...
            value = profile_eval_piece_value(piece->type);
...
static int profile_eval_pressure_sum(const CfBoard *board,
                                     const CfGasState *gas)
{
    const CfPiece *piece;
    int file;
    ...
    for (rank = 0; rank < 8; ++rank) {
        for (file = 0; file < 8; ++file) {
            ...
            sign = piece->color == CF_COLOR_WHITE ? 1 : -1;
            score += sign *
                profile_eval_pressure(board, file, rank, piece->color);
        }
    }
    return score;
}
...
{
    int score = 0;
    if (board_square_is_attacked(board, 4, 0, CF_COLOR_BLACK))
        score -= 35;
    if (board_square_is_attacked(board, 4, 7, CF_COLOR_WHITE))
        score += 35;
    return score;
}

static void run_eval_breakdown_profile(void)
{
    CfBoard board;
    CfGasState gas;
    clock_t start;
    ...
    unsigned long base_start_ms;
    unsigned long pressure_start_ms;
    unsigned long checks_start_ms;
    ...
    start = clock();
    for (repeat = 0; repeat < 500000; ++repeat)
        eval_breakdown_profile_sink += cpu_internal_evaluate(&board, &gas);
    end = clock();
    full_start_ms =
        (unsigned long)(((end - start) * 1000L) / CLOCKS_PER_SEC);
    ...
    start = clock();
    for (repeat = 0; repeat < 500000; ++repeat)
        eval_breakdown_profile_sink += profile_eval_base_scan(&board, &gas);
    end = clock();
    base_start_ms =
        (unsigned long)(((end - start) * 1000L) / CLOCKS_PER_SEC);
    ...
    for (repeat = 0; repeat < 500000; ++repeat)
        eval_breakdown_profile_sink +=
            profile_eval_pressure_sum(&board, &gas);
    end = clock();
    pressure_start_ms =
        (unsigned long)(((end - start) * 1000L) / CLOCKS_PER_SEC);
...
#ifndef CF_PROFILE_RUNTIME
static void run_timeout_stress_case(const char *name,
                                    CfCpuDifficulty difficulty)
{
    ...
    if (!cpu_choose_action(
        ...
        printf("TIME_STRESS %s ERROR no-action\n", name);
        return;
    }
    end = clock();
    elapsed_ms =
        (unsigned long)(((end - start) * 1000L) / CLOCKS_PER_SEC);
    printf("TIME_STRESS %s limit_ms=%lu elapsed_ms=%lu "
           "nodes=%lu depth=%d cutoffs=%lu score=%d budget=%d type=%d\n",
           name, config.time_limit_ms, elapsed_ms,
           stats.nodes, stats.depth_completed, stats.cutoffs,
           stats.score, stats.budget_hit, (int)action.type);
}

static void run_timeout_stress_profile(void)
{
    run_timeout_stress_case("EASY_100", CF_CPU_EASY);
    run_timeout_stress_case("MED_500", CF_CPU_MEDIUM);
    run_timeout_stress_case("HARD_1500", CF_CPU_HARD);
}
#endif
...
{
    const CfCpuProfileCounters *c;
    c = cpu_internal_profile_counters();
    printf("SEARCH_COUNTS %s full_budget=%lu recursive_budget=%lu "
           "clock_polls=%lu node_guard=%lu action_gen=%lu sort=%lu "
           "legal_probe=%lu eval=%lu move_attempt=%lu fart_attempt=%lu "
           "fart_bonus=%lu\n",
           name,
           c->full_budget_checks, c->recursive_budget_checks,
           c->clock_polls, c->node_guard_checks,
           c->action_gen_calls, c->sort_calls,
           c->legal_probe_calls, c->eval_calls,
           c->move_attempts, c->fart_attempts,
           c->fart_bonus_calls);
}

static void run_weighted_search_case(const char *name,
                                     CfCpuDifficulty difficulty,
                                     int gas_heavy)
{
    CfBoard board;
    ...
    if (cpu_choose_action(
            &board, &gas, &history, &config, &action, &stats))
        print_search_counters(name);
}

static void run_weighted_search_profile(void)
{
    run_weighted_search_case("HARD_START_TIMED", CF_CPU_HARD, 0);
    run_weighted_search_case("FART_HEAVY_MED_TIMED", CF_CPU_MEDIUM, 1);
}
#endif
...
#ifndef CF_PROFILE_RUNTIME
    run_timeout_stress_profile();
#else
    run_weighted_search_profile();
#endif
    run_perft_profile();
