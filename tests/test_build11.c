...
static void test_layout_contract(void)
{
    ...
    CHECK(CF_UI_SQUARE_SIZE == 22);
    CHECK(CF_UI_BOARD_PIXELS == 176);
    ...
    CHECK(CF_UI_BOARD_FRAME_X + CF_UI_BOARD_FRAME_W <= CF_UI_PANEL_X);
    CHECK(CF_UI_BOARD_FRAME_Y + CF_UI_BOARD_FRAME_H <= CF_UI_COMMAND_Y);
    CHECK(CF_UI_PANEL_X + CF_UI_PANEL_W <= CF_UI_SCREEN_W);
    CHECK(CF_UI_COMMAND_Y + CF_UI_COMMAND_H == CF_UI_SCREEN_H);
    CHECK(CF_UI_BOARD_PIXELS > CF_UI_PANEL_W);
    CHECK(CF_UI_BOARD_PIXELS * CF_UI_BOARD_PIXELS >
          CF_UI_PANEL_W * CF_UI_PANEL_H);
    CHECK(CF_UI_PANEL_CONTENT_X == 202);
    CHECK(CF_UI_PANEL_CONTENT_W == 105);
