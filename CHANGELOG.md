...
- 14.3 advances newly written game saves to format version 2 with an explicit CPU/local mode record. Version-1 saves remain accepted and restore CPU mode, preserving the semantics of saves created before local play existed. Version-2 mode restoration occurs before load-time CPU turn handling.
- 14.4 hardens local edge cases with real Chromium Black Farts, White/Black ordinary promotion, White/Black Fart-push promotion, and checkmate delivery/terminal locks for both colors. Ordinary promotion now has a dedicated command prompt and `ESC` cancels the choice instead of exiting the match. Board mouse clicks are ignored while ordinary or Fart-promotion choice is pending so mouse auto-target input cannot cycle or confirm the chooser. The native visual suite expands from 24 to 32 states.
- 14.5 adds a full 33-ply local two-player Opera Game Chromium gate from the normal title/start position to checkmate. The run uses real mouse moves for both colors, proves queenside castling and intermediate check states, performs a mid-game version-2 LOCAL save/load rollback of the complete persisted game state and two-ply replay, opens the real action-history UI, and rechecks terminal locking at mate. The existing Release CI remains the paired strict host/Open Watcom/DOSBox/package regression gate.
- 14.6 closes Build 14 with repository/package documentation consistency, final compatibility/release-checklist coverage, and permanent release-audit guards against stale RC control metadata or loss of the local-mode/v2-save documentation contract. No gameplay, CPU, persistence-format, audio, asset, layout or version-number changes are introduced.
